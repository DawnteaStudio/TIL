...
for (first,last) in keyword:
    ...
    while count_year < 21:
        ...
        # 뉴스 1페이지에서 링크는 직접 입력 (안해도 될 수도 있음.)
        driver.get(url)
        try:
            ...
            if no_news == f'intitle:{first} and intitle:{last} - Google 검색':
                if month < 12:
                    ws2.append([years,month,how_many,esg,f"{first}+' '+{last}"])
                    month += 1
                    count_month += 1
                    continue
                else:
        ...
        for article in articles:
            ...
            ws1.append([title,esg,years,month,f"{first}+' '+{last}"])
            how_many += 1
        ...
        ws2.append([f"{years}/{month}",how_many,esg,f"{first}+' '+{last}"])
